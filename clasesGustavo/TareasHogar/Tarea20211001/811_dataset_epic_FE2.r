...
#8 vCPU
#probando push con este script

#SE AGREGARON MAS VARIABLES AL FE. MAS VARIABLES TIPO COCIENTE
#LAS PALANCAS ESTÁN TAL CUAL EL SCRIPT ORIGINAL
...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , cp_cantidadprod          := rowSums( cbind( ccuenta_corriente,  ccaja_ahorro, ctarjeta_visa, ctarjeta_master, ctarjeta_debito ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidadprestamos          := rowSums( cbind( cprestamos_personales,  cprestamos_prendarios, cprestamos_hipotecarios ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidadinversiones          := rowSums( cbind( cplazo_fijo,  cinversion1, cinversion2 ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidadserv_ban          := rowSums( cbind( cseguro_vida,  cseguro_auto, cseguro_accidentes_personales, cseguro_vivienda ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_deb_aut_tc          := rowSums( cbind( ctarjeta_visa_debitos_automaticos,  ctarjeta_master_debitos_automaticos ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_deb_aut_tc_td          := rowSums( cbind( ccuenta_debitos_automaticos, ctarjeta_visa_debitos_automaticos,  ctarjeta_master_debitos_automaticos ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_pago_serv          := rowSums( cbind( cpagodeservicios,  cpagomiscuentas ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_descuentos_td_tc         := rowSums( cbind( ccajeros_propios_descuentos,  ctarjeta_visa_descuentos,ctarjeta_master_descuentos  ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_descuentos_tc          := rowSums( cbind(  ctarjeta_visa_descuentos,ctarjeta_master_descuentos  ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_comisiones          := rowSums( cbind( ccomisiones_mantenimiento,  ccomisiones_otras  ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_trans          := rowSums( cbind( ctarjeta_debito_transacciones,  ctarjeta_visa_transacciones, ctarjeta_master_transacciones   ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_payroll          := rowSums( cbind( cpayroll_trx,  cpayroll2_trx  ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_forex          := rowSums( cbind(  cforex_buy, cforex_sell ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_transfer         := rowSums( cbind( ctransferencias_recibidas, ctransferencias_emitidas ) , na.rm=TRUE ) ]
  dataset[ , cp_cantidad_trans_caja         := rowSums( cbind( ccajas_transacciones, ccajas_consultas, ccajas_depositos,ccajas_extracciones,ccajas_otras   ) , na.rm=TRUE ) ]
  
  
  dataset[ , mp_consumo_tc         := rowSums( cbind( mtarjeta_visa_consumo,  mtarjeta_master_consumo ) , na.rm=TRUE ) ]
  dataset[ , mp_consumo_tc_td         := rowSums( cbind( mtarjeta_visa_consumo,  mtarjeta_master_consumo, mautoservicio ) , na.rm=TRUE ) ]
  dataset[ , mp_prestamos         := rowSums( cbind( mprestamos_personales,  mprestamos_prendarios, mprestamos_hipotecarios ) , na.rm=TRUE ) ]
  dataset[ , mp_invers_dolar         := rowSums( cbind( mplazo_fijo_dolares,  minversion1_dolares ) , na.rm=TRUE ) ]
  dataset[ , mp_invers_pesos         := rowSums( cbind( mplazo_fijo_pesos,  minversion1_pesos, minversion2 ) , na.rm=TRUE ) ]
  dataset[ , mp_tc_debitos_auto         := rowSums( cbind( mttarjeta_visa_debitos_automaticos,  mttarjeta_master_debitos_automaticos ) , na.rm=TRUE ) ]
  dataset[ , mp_tc_td_debitos_auto         := rowSums( cbind( mcuenta_debitos_automaticos, mttarjeta_visa_debitos_automaticos,  mttarjeta_master_debitos_automaticos ) , na.rm=TRUE ) ]
  dataset[ , mp_monto_desc_td_tc         := rowSums( cbind( mcajeros_propios_descuentos, mtarjeta_visa_descuentos,  mtarjeta_master_descuentos ) , na.rm=TRUE ) ]
  dataset[ , mp_monto_desc_tc         := rowSums( cbind( mtarjeta_visa_descuentos,  mtarjeta_master_descuentos ) , na.rm=TRUE ) ]
  dataset[ , mp_pago_serv         := rowSums( cbind( mpagodeservicios,  mpagomiscuentas) , na.rm=TRUE ) ]
  dataset[ , mp_monto_comisiones        := rowSums( cbind( mcomisiones_mantenimiento,  mcomisiones_otras) , na.rm=TRUE ) ]
  dataset[ , mp_monto_forex        := rowSums( cbind( mforex_buy,  mforex_sell) , na.rm=TRUE ) ]
  dataset[ , mp_monto_cc_ca         := rowSums( cbind( mcuenta_corriente_adicional,  mcuenta_corriente, mcaja_ahorro, mcaja_ahorro_adicional) , na.rm=TRUE ) ]
  dataset[ , mp_monto_payroll        := rowSums( cbind( mpayroll,  mpayroll2) , na.rm=TRUE ) ]
  dataset[ , mp_monto_ca         := rowSums( cbind(  mcaja_ahorro, mcaja_ahorro_adicional) , na.rm=TRUE ) ]
  
  #Razón de consumo de tarjetas e ingreso
  dataset[ , consumo_ingreso:= mp_consumo_tc_td / mp_monto_payroll ]
  
  #Razón de consumo de tarjetas e ingreso
  dataset[ , consumo_tc_ingreso:= mp_consumo_tc / mp_monto_payroll ]
  
  #razón prestamo ingreso
  dataset[ , prestamos_ingreso:= mp_prestamos / mp_monto_payroll ]
  
  #razón saldo en cuenta ingresos
  
  dataset[ , saldo_ingreso:= mp_monto_cc_ca / mp_monto_payroll ]
  
  #razón caja de ahorro ingresos
  dataset[ , saldo_ca_ingreso:= mp_monto_ca / mp_monto_payroll ]
  
  #razón consumo tc-td e inversiones en pesos
  
  dataset[ , consumo_inversión:= mp_consumo_tc_td / mp_invers_pesos ]
  
  #razón consumo tc e inversiones en pesos
  
  dataset[ , consumo_tc_inversión:= mp_consumo_tc / mp_invers_pesos ]
  
  #razón de transacciones en forex
  dataset[ , forex_tasa:= mp_monto_forex / cp_cantidad_forex ]
  
  #razón de inversion pesos - ingreso
  dataset[ , inv_pesos_ingreso:= mp_invers_pesos  / mp_monto_payroll ]
  
  #razón de inversion pesos - ingreso
  dataset[ , inv_dolar_ingreso:= mp_invers_dolar  / mp_monto_payroll ]
  
  #razón de forex - ingreso
  dataset[ , forex_ingreso:= mp_monto_forex  / mp_monto_payroll ]

  #razón de servicios - ingreso
  dataset[ , serv_ingreso:= mp_pago_serv / mp_monto_payroll ]
  
  #razón de debitos tc - ingreso
  dataset[ , tc_debitos_auto_ingreso:= mp_tc_debitos_auto / mp_monto_payroll ]
  
  #razón de debitos tc_td - ingreso
  dataset[ , tc_td_debitos_auto_ingreso:= mp_tc_td_debitos_auto / mp_monto_payroll ]
  
  #razon transacciones y transacciones de caja
  dataset[ , trans_trans_caja:= cp_cantidad_trans  / cp_cantidad_trans_caja ]
...
correr_todo  <- function( palancas )
{
  ...
  #Grabo el dataset
  fwrite( dataset,
          paste0( "./datasets/dataset_epic_adding", palancas$version, ".csv.gz" ),
          logical01 = TRUE,
          sep= "," )
