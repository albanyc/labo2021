...
kscript         <- "822_epic_V006"

karch_dataset    <- "./datasets/dataset_epic_addingv006.csv.gz"   #este dataset se genero en el script 811_dataset_epic_adding.r
...
hs <- makeParamSet( 
         ...
         makeIntegerParam("num_leaves",       lower=  50L   , upper= 1100L)
        )

campos_malos  <- c()   #aqui se deben cargar todos los campos culpables del Data Drifting

ksemilla_azar  <- 471901  #Aqui poner la propia semilla
#------------------------------------------------------------------------------
